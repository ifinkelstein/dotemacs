...
The goal is to use this with GPTel to query PDF files via an LLM."
    (interactive)
...
system's default application."
    ...
    (let ((gptel-model "Claude:claude-3-haiku-20240307")) ;; earlier model: gpt-4-0125-preview
      (message (concat "Sending query using " gptel-model))
      (gptel-request
          (concat "The text below is about scheduling a meeting. Summarize the following text in an RFC5545 compliant text response with the meeting date, time, and location. Do not add any additional characters to the top or last line of the response. Again, the response should be able to be parsed by any RFC5545 compliant calendar software. If the meeting is via zoom, include a zoom link only when it was mentioned. Include a time zone. If no time zone is defined, assume central time (Chicago). If no year is defined, assume "
                  (format-time-string "%Y")
                  ". The host or organizer should be Ilya Finkelstein with the email ilya@finkelsteinlab.org. In the notes field, include a concise synopsis of the topic of the meeting. Limit the synopsis to three sentences, maximum. Start the synopsis with [Generated by "
                  gptel-model
                  " on "
