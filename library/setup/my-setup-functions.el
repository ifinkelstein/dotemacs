...
(defun my-setup-kill-and-archive-region ()
  "Delete & append region to end of setup-archive.el"
  (interactive)
  (append-to-file (region-beginning) (region-end) (concat my-setup-dir "my-setup-archive.el"))
...
;; TODO: Update path in rg to point to my setup
...
(defun my-weather-journal-capture ()
  (interactive)
  (progn
    (require 'org)
    (select-frame-by-name "capture")
...
(bind-key "C-c m" #'my-goto-minibuffer-window)
...
(bind-key* "C-x n n" #'my-narrow-or-widen-dwim narrow-map)
...
(defun my-insert-time-seconds-epoch ()
  "Insert the integer number of seconds since the epoch."
  (interactive)
  (with-temp-buffer
    (insert (format-time-string "%s"))
    (buffer-string)))
;; (global-set-key (kbd "C-c e") 'my-insert-time-seconds-epoch)
...
(defun my-insert-commented-separator()
